...
export class HitsListingComponent implements OnInit {
  ...
  constructor(private commonService: CommonService, private matDialog : MatDialog) {
    ...
    //   .pipe(
    //     startWith(0),
    //     map(() => { this.commonService.getHitsListing() })
    //   ).subscribe((hits) => {
    //     if(typeof hits != typeof undefined){
    //       this.dataSource = new MatTableDataSource(hits);
    //       this.init();
    //     }
    //     }
    //   );
    this.commonService.getHitsListing().subscribe((hits) => {
